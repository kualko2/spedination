# Spedination Brand Guidelines

> **For AI assistants:** Read this document completely before generating any visual output, documents, or content for Spedination GmbH or Spedination Zillertal GmbH. All rules below are mandatory.

**Base URL for all assets:**
```
https://raw.githubusercontent.com/kualko2/brand/main/
```

---

## 1. Companies

### Spedination GmbH
| Field | Value |
|---|---|
| Address | Sportplatzweg 5a, A-6336 Langkampfen, Austria |
| Email | office@spedination.com |
| Phone | +43 5372 21960 |
| Managing Director | Thomas Kogler |
| UID-Nr. | ATU75905969 |
| Firmenbuchnummer | FN 540077g |
| Firmenbuchgericht | Bezirksgericht Kufstein |
| Bank | Sparkasse Kufstein |
| IBAN | AT34 2050 6077 0012 8692 |
| BIC | SPKUAT22XXX |
| TIMOCOM ID | 398042 |

### Spedination Zillertal GmbH
| Field | Value |
|---|---|
| Address | Sportplatzweg 3, A-6270 Uderns, Austria |
| Email | zillertal@spedination.com |
| Phone | +43 5372 21960400 |
| Managing Directors | Thomas Kogler, Mario Wildauer |
| UID-Nr. | ATU77762967 |
| Firmenbuchnummer | FN 571810y |
| Firmenbuchgericht | Bezirksgericht Schwaz |
| Bank | Sparkasse Kufstein |
| IBAN | AT40 2050 6077 0014 9342 |
| BIC | SPKUAT22XXX |
| TIMOCOM ID | 433281 |

### Web & Legal
| | |
|---|---|
| Website | www.spedination.com |
| AGB | https://www.spedination.com/agb |
| Impressum | https://www.spedination.com/impressum |
| Datenschutz | https://www.spedination.com/datenschutz |

---

## 2. Logo

### Logo Files

| File | Description | URL |
|---|---|---|
| `Spedination-Logo-positiv-RGB.png` | Standard – for light backgrounds | [↓](https://raw.githubusercontent.com/kualko2/brand/main/Spedination-Logo-positiv-RGB.png) |
| `Spedination-Logo-negativ-RGB.png` | Negative – for dark backgrounds | [↓](https://raw.githubusercontent.com/kualko2/brand/main/Spedination-Logo-negativ-RGB.png) |
| `Spedination-Logo-1c schwarz-RGB.png` | 1-color black | [↓](https://raw.githubusercontent.com/kualko2/brand/main/Spedination-Logo-1c%20schwarz-RGB.png) |
| `Spedination-Logo-1c weiss-RGB.png` | 1-color white | [↓](https://raw.githubusercontent.com/kualko2/brand/main/Spedination-Logo-1c%20weiss-RGB.png) |
| `Spedination-Logo-alternativ-RGB.png` | Alternative layout | [↓](https://raw.githubusercontent.com/kualko2/brand/main/Spedination-Logo-alternativ-RGB.png) |

### Logo Rules – MANDATORY
- Logo text: **ONLY Deep Black (#101921) or Plain White (#ffffff)** – never any other color
- **Documents** (letters, forms, instructions): logo **top LEFT**
- **Advertising** (banners, social media, Werbesujets): logo **bottom RIGHT**
- Minimum width: 20mm print / 70px screen
- Never rotate, distort, recolor, or add effects
- "SPEDINATION" = FF Tisa Sans Bold | "moves" = Fira Sans Italic

---

## 3. Colors

### Primary
| Name | HEX | RGB | CMYK |
|---|---|---|---|
| **Deep Black** | `#101921` | 16, 25, 33 | 100-61-32-96 |
| **Lemon Yellow** | `#e2e200` | 226, 226, 0 | 7-0-100-0 |
| **Plain White** | `#ffffff` | 255, 255, 255 | 0-0-0-0 |

### Secondary
| Name | HEX | RGB | CMYK |
|---|---|---|---|
| **Lime Green** | `#93ca15` | 147, 202, 21 | 43-0-100-0 |
| **Tree Green** | `#509f33` | 80, 159, 51 | 66-0-100-9 |

### Tertiary
| Name | HEX | RGB | CMYK |
|---|---|---|---|
| **Berry Pink** | `#7e1c4e` | 126, 28, 78 | 11-100-9-47 |
| **Plum Blue** | `#3d3d5f` | 61, 61, 95 | 79-72-17-43 |
| **Deep Ocean** | `#2e6f7e` | 46, 111, 126 | 78-28-30-27 |
| **Street Grey** | `#545860` | 84, 88, 96 | 63-52-44-33 |

### Color Rules
- Lemon Yellow = dominant brand color
- Secondary/tertiary colors: **never in the logo**
- Photos with text/logo: desaturate → Deep Black overlay 20–40% opacity
- **Lemon Yellow photo filter: never allowed**

---

## 4. Typography

| Role | Font | Weight | Size |
|---|---|---|---|
| Main heading | FF Tisa Sans | Bold | 36pt |
| Sub-heading | Fira Sans | Italic | 14pt |
| Intermediate heading | FF Tisa Sans | Bold | 18pt |
| Body text | FF Tisa Sans | Regular | 10pt |
| Quiet emphasis | Fira Sans | Italic | 9pt |
| Small print / AGB | FF Tisa Sans | Regular | 6.5pt |
| **Office / Windows** | **Lucida Sans** | Regular | – |
| **Fallback** | **Arial** | Regular | – |

**Rules:** Left-aligned, ragged right, no hyphenation. Large headers sit above a Lemon Yellow rule line. Office documents always use Lucida Sans.

---

## 5. SSE – Secondary Style Elements

Rounded corner brackets from the logo icon. Visual constant across all Spedination communications.

### Yellow SSE (standard)

| File | Position | URL |
|---|---|---|
| `Spedination-SSE-gelb-links oben-RGB.png` | Top left | [↓](https://raw.githubusercontent.com/kualko2/brand/main/Spedination-SSE-gelb-links%20oben-RGB.png) |
| `Spedination-SSE-gelb-rechts oben-RGB.png` | Top right | [↓](https://raw.githubusercontent.com/kualko2/brand/main/Spedination-SSE-gelb-rechts%20oben-RGB.png) |
| `Spedination-SSE-gelb-links unten-RGB.png` | Bottom left | [↓](https://raw.githubusercontent.com/kualko2/brand/main/Spedination-SSE-gelb-links%20unten-RGB.png) |
| `Spedination-SSE-gelb-rechts unten-RGB.png` | Bottom right | [↓](https://raw.githubusercontent.com/kualko2/brand/main/Spedination-SSE-gelb-rechts%20unten-RGB.png) |
| `Spedination-SSE-gelb-Mitte-RGB.png` | Detail / focus | [↓](https://raw.githubusercontent.com/kualko2/brand/main/Spedination-SSE-gelb-Mitte-RGB.png) |

### White SSE (exception only)

| File | Position | URL |
|---|---|---|
| `Spedination-SSE-weiss-links oben-RGB.png` | Top left | [↓](https://raw.githubusercontent.com/kualko2/brand/main/Spedination-SSE-weiss-links%20oben-RGB.png) |
| `Spedination-SSE-weiss-rechts oben-RGB.png` | Top right | [↓](https://raw.githubusercontent.com/kualko2/brand/main/Spedination-SSE-weiss-rechts%20oben-RGB.png) |
| `Spedination-SSE-weiss-links unten-RGB.png` | Bottom left | [↓](https://raw.githubusercontent.com/kualko2/brand/main/Spedination-SSE-weiss-links%20unten-RGB.png) |
| `Spedination-SSE-weiss-rechts unten-RGB.png` | Bottom right | [↓](https://raw.githubusercontent.com/kualko2/brand/main/Spedination-SSE-weiss-rechts%20unten-RGB.png) |
| `Spedination-SSE-weiss-Mitte-RGB.png` | Detail / focus | [↓](https://raw.githubusercontent.com/kualko2/brand/main/Spedination-SSE-weiss-Mitte-RGB.png) |

### SSE Rules
- Default: **always Lemon Yellow**
- White: only when Lemon Yellow doesn't work on the background
- Never use SSE in secondary or tertiary colors
- Documents: all four corners within printable margin (~8mm from physical edge)
- Multi-line highlights: all four corners
- Single-line highlights: bottom-left + top-right only
- Middle element: to highlight a photo detail

---

## 6. Document Layout

### Header
```
[LOGO – top left]                    [Document title]
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ (Lemon Yellow 3px)
```

### Footer
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ (Grey 1px)
Spedination GmbH · Sportplatzweg 5a, A-6336 Langkampfen | Spedination Zillertal GmbH · Sportplatzweg 3, A-6270 Uderns | www.spedination.com
```

---

## 6b. Design Patterns (AI Reference)

Standard UI components used across all Spedination documents and HTML:

### Step Badge
- Lemon Yellow square/rectangle + step number in Deep Black Bold
- Description text right of badge in `#f8f8f8` background

### Section Title Bar
- Deep Black background, Plain White bold text
- Optional Lemon Yellow accent word or left border

### Warning Box
- Left border: 5px Lemon Yellow
- Background: `#fffbe6`
- Text: bold, color `#7b4000`

### Info Box
- Left border: 4px Street Grey
- Background: `#f4f4f4`
- Text: Street Grey

### HTML Print Rule
- Always add `print-color-adjust: exact !important` to all elements
- Page size: `@page { size: A4; margin: 0; }`
- SSE corners: `position: absolute`, min `8mm` from physical page edge

### Logo handling in HTML/code
- The positiv PNG file has a **black background** (not transparent)
- To use on white backgrounds: remove black pixels programmatically (replace pixels where R<30, G<30, B<30 with alpha=0)
- The logo icon may be used standalone (without text) in any primary color, preferably Lemon Yellow


## 7. AI Assistant Checklist

- [ ] Logo top-left (documents) or bottom-right (advertising)
- [ ] Logo only in Deep Black or Plain White
- [ ] SSE: Lemon Yellow, all four corners
- [ ] Accent color: Lemon Yellow `#e2e200`
- [ ] Text: Deep Black `#101921`
- [ ] Subtext: Street Grey `#545860`
- [ ] Font: FF Tisa Sans → Lucida Sans (Office) → Arial (fallback)
- [ ] Footer: both company addresses
- [ ] Warning boxes: Lemon Yellow left border, background `#fffbe6`
- [ ] HTML/CSS print: `print-color-adjust: exact`

---

## 8. Brand Tone

- Direct, concise, honest – "Handschlagqualität"
- Young and dynamic (target: 16–35 years old)
- Informal ("per du" in German)
- Networker mindset, digitally forward
- Industry: FTL transport, international freight forwarding, DACH + Eastern Europe

---

*[github.com/kualko2/brand](https://github.com/kualko2/brand) · 2026*
